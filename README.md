...
I am researching social media and mental health. I choose to evaluate the “gender” column within the dataset. I want to see if there is any difference in the happiness of social media users based on the gender the participate. 
...
* The test statistic for your permutation test
	Create "Happiness" coulmn from "Happiness_Index(1=10)". Created function to get observed stat from orginal data subtracting number of unhappy men from happy men using the shape. Subtracted number of unhappy others from happy othersing using the shape. Then I subtracted the unhappy others from unhappy men to get the observed stat. The observed stat will be my test statistic for the permutation.
* How you simulated or resampled under the null hypothesis
	I generated the null distribution by shuffling the Gender column while keeping thier happiness responses fixed. For each shuffle I recalcuted the test statistic 1000 times.
* The metric(s) for which you created bootstrap confidence intervals
	I bootstrapped the Test_Statitic via the function and the median of the column "Happiness_Index(1-10)".
* Why the CLT does not apply to at least one metric
	Median of the column "Happiness_Index(1-10)" and CLT do not work well as median is a nonlinear statistic, where the CLT states that if you take a sample mean and grow the sample size, the distrubtion will normlize. 
